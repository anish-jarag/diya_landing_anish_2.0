    ...
    images.map(async (image) => {
      if (image?.url) {
        ...
        if (typeof _image === 'object' && _image) {
          return {
            url:
              'src' in _image && typeof (_image as { src?: string }).src === 'string'
                ? String(new URL((_image as { src: string }).src, astroSite))
                : '',
            width:
              'width' in _image && typeof (_image as { width?: number }).width === 'number'
                ? (_image as { width: number }).width
                : undefined,
            height:
              'height' in _image && typeof (_image as { height?: number }).height === 'number'
                ? (_image as { height: number }).height
                : undefined,
          };
        }
        return {
